...
void Handle_Add_Question(SOCKET socket,   char * MessageBuffer){


printf(" DANS HANDLE ADD QUESTION \n");
    char Quizz_Question[QUESTION_SIZE];
     
    memset(Quizz_Question, 0, QUESTION_SIZE);

    // acknowledge the client that I am ready to receive question

    send(socket, Quizz_Question,  MESSAGE_SIZE, 0 );

    memset(Quizz_Question, 0, QUESTION_SIZE);
    // receiving the actual question
    recv(socket , Quizz_Question, QUESTION_SIZE, 0 );

    printf("  la question recu est : %s \n ", Quizz_Question);      

      
     
    Add_Question_From_String( Quizz_Question );
}
...
void Action_Switch(int flag, int* Responseflag, char * MessageBuffer , SOCKET socket, int * Ptr_User_Id , int * Ptr_User_Score, char ** LocalQuestionMatrix ){
    printf(" bien dans do some action flag : %d \n", flag);
    switch (flag)
    {
    case 1:
         Handle_GameLunch();
    ...
    case 4:
        ...
        break;  
    case 5:
        printf(" ABOUT TO ADD QUESTION \n");
          Handle_Add_Question( socket, MessageBuffer);
        break;  
     case 8:
    ...
    default:
    printf(" no action do some action");
        break;
    }
    ...
    while (1) {
       ...
       ResponseFlag = 0;
        ...
        if (recvResult > 0) {
           ...
           printf(" flag a l'exterieur de la fonction switch : %d , ",  flag);
